/*
 * Copyright 2013 The Solmix Project
 *
 * This is free software; you can redistribute it and/or modify it
 * under the terms of the GNU Lesser General Public License as
 * published by the Free Software Foundation; either version 2.1 of
 * the License, or (at your option) any later version.
 *
 * This software is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU
 * Lesser General Public License for more details.
 *
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 * http://www.gnu.org/licenses/ 
 * or see the FSF site: http://www.fsf.org. 
 */
package org.solmix.hola.rs.support;

import org.solmix.commons.util.Assert;
import org.solmix.hola.rs.RemoteCall;


/**
 * 
 * @author solmix.f@gmail.com
 * @version $Id$  2014年5月17日
 */

public class RemoteCallImpl implements RemoteCall
{
    protected String method;
    protected Object[] parameters;
    protected long timeout;
    
    public RemoteCallImpl(String method,Object[] parameters,long timeout){
        this.method=method;
        this.parameters=parameters;
        this.timeout=timeout;
        Assert.isNotNull(this.method);
    }
    public RemoteCallImpl(String method,Object[] parameters){
        this(method,parameters,DEFAULT_TIMEOUT);
    }
    
    public RemoteCallImpl(String method){
        this(method,null);
    }
    /**
     * {@inheritDoc}
     * 
     * @see org.solmix.hola.rs.RemoteCall#getMethod()
     */
    @Override
    public String getMethod() {
        return method;
    }

    /**
     * {@inheritDoc}
     * 
     * @see org.solmix.hola.rs.RemoteCall#getParameters()
     */
    @Override
    public Object[] getParameters() {
        return parameters;
    }

    /**
     * {@inheritDoc}
     * 
     * @see org.solmix.hola.rs.RemoteCall#getTimeout()
     */
    @Override
    public long getTimeout() {
        return timeout;
    }

}
