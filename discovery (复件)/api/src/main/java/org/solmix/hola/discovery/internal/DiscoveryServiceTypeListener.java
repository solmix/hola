...
/**
 * 监听OSGI环境中注册的{@link org.solmix.hola.discovery.DiscoveryListener},并自动添加listener.
 * 只Spring如需添加需手动代码实现
 */
...
public class DiscoveryServiceTypeListener  implements ServiceListener
{
        ...
        if (context != null) {
            try {
                final ServiceReference<?>[] references = context.getServiceReferences(
                    org.solmix.hola.discovery.DiscoveryTypeListener.class.getName(),
                    null);
    ...
    private void addServiceListener(ServiceReference<?>[] references) {
        ...
        for (ServiceReference<?> refer : references) {
            final org.solmix.hola.discovery.DiscoveryTypeListener listener = (org.solmix.hola.discovery.DiscoveryTypeListener) context.getService(refer);
